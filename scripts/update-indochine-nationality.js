...
// Artist to update to FR
const artistToUpdate = 'Indochine';
const targetNationality = 'FR';
...
files.forEach(file => {
  ...
  videos.forEach(video => {
    if (video.artist_name === artistToUpdate) {
      if (video.nationality !== targetNationality) {
        console.log(`Updating ${video.artist_name} - "${video.song_title}" (${video.year}) from ${video.nationality} to ${targetNationality}`);
